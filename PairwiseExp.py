# PairwiseExp.py
#
# This is a variant of PPOexperiment.py; see its header comments for
# more about it.
#
# To run a second-order training experiment:
#
# * Run normal experiments using PPOexperiment.py making sure to
#   set the proper environment variables as described in README.md
# * Log the output of each experiment in a file with a name like
#   20230123.123456-log.txt or 20230123.123456-log.txt.gz
# * Put the logfiles in the directory ../logs
# * Run the script "survey.pl", which will generate the file
#   and give you instructions, after which you will have a list of
#   experiment logs in ./l-survey-list.txt
# * Run the script "gen-pairwise.pl", which will generate the file
#   ./p-pairwise-weights-pos.txt
...
# Load the pairwise hints data
pweights = []
def pairwise_load(passlist):
  global pweights
  pweights = ([0] * len(passlist))
  for row in range(len(passlist)):
    pweights[row] = ([0] * len(passlist))
  with open('p-pairwise-weights-pos.txt', 'r') as pairslist:
    #             286727.6 -scalarrepl -constmerge        0.010774
    p = re.compile(r"^ *([.0-9]+) +(-[a-z0-9]+) +(-[a-z0-9]+) +([.0-9]+)", re.DOTALL)
    pline = pairslist.readline()
    while pline != '':
      m = re.match(p, pline)
      if m:
        pass1 = m.group(2)
...
def configEnv_variables():   # called by __init__
    ...
    pairwise_load(env_config["opt_passes"])
...
class myEnv(gym.Env):
    ...
    def __init__(self):  # called by rlib
        ...
        print("%s init: max_ep_steps %d" % (d14(),   # so we know what VARIANT was
    ...
    def step(self, action):  # called by rlib
        ...
        if (not is_done) and PAIRWISE_HINTS:
          ...
          if actlen > 1 :
            ix1 = self.env_config["actions"][actlen-2]
